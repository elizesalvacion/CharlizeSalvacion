...
function initSkillsHover() {
    ...
    skillLabels.forEach(label => {
        label.addEventListener('mouseenter', function() {
            ...
            const targetDetail = document.querySelector(`.skills-detail[data-detail="${skillType}"]`);
            if (targetDetail) {
        ...
        label.addEventListener('mouseleave', function() {
            setTimeout(() => {
                const isHoveringAnyLabel = Array.from(skillLabels).some(lbl => lbl.matches(':hover'));
...
function initHeroEyes() {
    ...
    const eyeCenter = {
        left: { x: leftBox.x + leftBox.width / 2, y: leftBox.y + leftBox.height / 2 },
        right: { x: rightBox.x + rightBox.width / 2, y: rightBox.y + rightBox.height / 2 },
    };
    ...
    const setEyes = (dx, dy) => {
        ...
        heroLeftEye.setAttribute('transform', `translate(${dx * leftWeightX} ${dy})`);
        heroRightEye.setAttribute('transform', `translate(${dx * rightWeightX} ${dy})`);
    };
...
function initSparkleCursor() {
    ...
    window.addEventListener('pointerdown', () => {
        glow.style.boxShadow = '0 0 0 8px rgba(180, 173, 234, 0.22), 0 0 34px rgba(180, 173, 234, 0.65)';
    }, { passive: true });

    window.addEventListener('pointerup', () => {
        glow.style.boxShadow = '0 0 0 6px rgba(180, 173, 234, 0.18), 0 0 28px rgba(180, 173, 234, 0.55)';
    }, { passive: true });
